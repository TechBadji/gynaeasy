...
import { validatePatientCode, createOnlineAppointment } from "@/app/actions/booking";
import { prisma } from "@/lib/prisma"; // This won't work on client, I need a server action to get doctors
...
export default function BookingPage() {
    ...
    const handleValidateCode = async () => {
        if (patientCode.length < 5) return;
        startTransition(async () => {
            const patient = await validatePatientCode(patientCode);
            if (patient) {
                ...
                {step === 1 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="text-center md:text-left">
                            ...
                            <p className="text-slate-500 font-medium">Choisissez un spécialiste pour votre consultation.</p>
                        </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            {doctors.map((doc) => (
                                <button
                                    ...
                                    className="p-6 bg-white border border-slate-200 rounded-3xl text-left hover:border-violet-500 hover:shadow-xl transition-all group"
                                >
                                    <div className="flex items-start gap-4">
                                        <div className="h-16 w-16 rounded-2xl bg-slate-100 flex items-center justify-center text-2xl font-black text-slate-400 group-hover:bg-violet-50 group-hover:text-violet-500 transition-colors">
                                            {doc.name?.charAt(0)}
                                        </div>
                                        <div className="flex-1">
                                            <h3 className="text-xl font-bold group-hover:text-violet-600 transition-colors">{doc.name}</h3>
                ...
                {step === 3 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500 max-w-xl mx-auto">
                        ...
                        <div className="bg-white border border-slate-200 rounded-[2.5rem] p-10 shadow-sm text-center">
                            ...
                            <p className="text-slate-500 mb-8 font-medium">Entrez votre code patient à 5 chiffres pour confirmer.</p>
                            ...
                            <div className="space-y-6">
                                <div className="relative">
                                    <input
                                        ...
                                        maxLength={5}
                                        placeholder="#####"
                                        className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-6 text-center text-4xl font-black tracking-[1rem] focus:outline-none focus:border-violet-500 transition-all placeholder:text-slate-100"
                                        value={patientCode}
