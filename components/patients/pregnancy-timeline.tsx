    ...
    return (
        <div className="space-y-6">
            <div className="p-4 bg-violet-50 border border-violet-100 rounded-lg flex items-center justify-between">
                <div>
                    <h3 className="text-lg font-bold text-violet-900">Terme actuel : {sa} SA + {jours} jours</h3>
                    <p className="text-sm text-violet-700">DDR : {ddr.toLocaleDateString('fr-FR')}</p>
                </div>
                ...
                {EXAMENS_GROSSESSE.map((examen, i) => {
                    ...
                    return (
                        <div key={i} className={`relative pl-6 ${isPassed ? 'opacity-50' : ''}`}>
                            <div className={`absolute -left-[9px] top-1 h-4 w-4 rounded-full border-2 ${isCurrent ? 'bg-violet-500 border-violet-200 ring-4 ring-violet-100' :
